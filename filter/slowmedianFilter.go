...
//
//// the reason this code is so fucking slow is because its not running in parallel at all
//// its running everything in one channel
//// to fix it dont do everything in one loop
//// do the split in one loop
//// the receive  in one loop
//// then merge in one loop
////that way it can do it in parrallel
//
//import (
//	"flag"
//	"image"
...
//	//var channels []chan [][]uint8 // create slices of channel where each channel transmit a 2d slice
//	channels := make([]chan [][]uint8, threads) // need to use threads if not it wont it will stay empty ? -------------------------------!
//	parts := make([][][]uint8, threads)         // a slice that holds multiple 2D slice , if threads == 8 it will create 8
//
//	if threads == 1 {
//		newPixelData = medianFilter(0, height, 0, width, immutableData)
//	} else {
//		for i := 0; i < threads; i++ {
//			channels[i] = make(chan [][]uint8) // create channels in the loop if not you wont create loop  -------------------------------!
//			j := height / threads
//			go worker(i*(j), (i*(j))+j, 0, width, immutableData, channels[i]) // straight away put them into the already initiase channels
//			parts[i] = <-channels[i]
